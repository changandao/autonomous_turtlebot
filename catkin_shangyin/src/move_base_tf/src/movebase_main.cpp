...
//            }else{
                linear_x = double(0.4 * sqrt(pow(x_base_goal, 2) + pow(y_base_goal, 2)));
//            }

            // check if near goal
            // mark: niu bi li taet
            if (std::abs(y_base_goal)<0.03 && std::abs(x_base_goal)<0.03 && movebase.use_rearly_stop == true){
                vel_msg.angular.z = 0;
                vel_msg.linear.x = 0;
                movebase.reach_goal = true;
                //movebase.reach_goal_theta = false;
            ...
            if(std::abs(theta_base_goal)<0.05){
                vel_msg.angular.z = 0;
                vel_msg.linear.x = 0;
