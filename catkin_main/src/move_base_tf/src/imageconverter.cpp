...
int ImageConverter::return_detected_signal(){
    return puck_color_detected;
}


void ImageConverter::get_puck_catch_signal(bool get_puck_flag)
{
    puck_catched = get_puck_flag;
}


bool ImageConverter::return_catching_puck_signal()
{
    bool catching_puck_signal = catching_puck;
    return catching_puck_signal;
}
...
void ImageConverter::recognize(Mat img_in, Mat out_img, Mat im_gray, Mat drawing, int iLowH, int iHighH, int iLowS, int iLowV, int flag)
{
    ...
    if (flag == 1){
        ...
        if(obj_size > 5000)
        {
            puck_color_detected = 1.0;
            ball_center_pos = boundRect[imax].x + boundRect[imax].width/2;
            ball_pos.x = ball_center_pos;
            ball_pos.y = puck_color_detected;/*boundRect[imax].y + boundRect[imax].height/2;*/
            ...
            if(puck_catched){
                if (obj_size > 1000000) catching_puck = true;
                else catching_puck = false;
            }
        }
        else{
            puck_color_detected = 0;
            image_lookup = true;
