...
Image2World::Image2World(){
    ...
    pointttt = nh.advertise<geometry_msgs::PointStamped>("/my_topics/world3d",10);
}
        ...
        if (x>20)
        {
            ...
            if (!isnan(range_min_angle))
            {
                ...
                if((p.point.x!=0)|(p.point.y!=0)){
                    ...
                    cb_count++;
                    if(flag == 1) pointttt.publish(ppp);
                    else{
                        p.point.x = 0;
                        p.point.y = 0;//-= 0.2*ratio + 0.05;
                        p.point.z = 0;//theta + (double(x) - 320.0)/320.0 * angle_max;
